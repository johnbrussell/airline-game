...
RSpec.describe Airplane do
  ...
  context "buy_out_lease" do
    let(:purchase_price_new) { 10000000 }
    let(:initial_cash_on_hand) { 100000000 }
    let(:initial_lease_rate) { 10000 }
    let(:initial_lease_expiry) { Game.find(operator.game_id).current_date + 1.day }
    ...
    let(:subject) { Fabricate(
      ...
      owner_id: nil,
      construction_date: Game.find(operator.game_id).current_date - 1.day,
      lease_expiry: initial_lease_expiry,
      lease_rate: initial_lease_rate
    ) }

    it "returns false when the airline does not have enough cash on hand" do
      operator.update(cash_on_hand: 1)

      expect(subject.buy_out_lease).to be false
      ...
      expect(operator.cash_on_hand).to eq 1
      expect(subject.lease_rate).to eq initial_lease_rate
      expect(subject.lease_expiry).to eq initial_lease_expiry
      expect(Airplane.with_operator(operator)).to include subject
      expect(subject.errors.full_messages).to include "Operator does not have enough cash on hand to buy out lease"
    end

    it "returns false when the plane is already owned" do
    ...
    it "returns false when the plane is not built" do
    ...
    it "returns true and updates the ownership information without impacting flights in operation" do
      ...
      expected_purchase_price = model.price * model.lease_buyout_premium

      expect(subject.buy_out_lease).to be true
      ...
      expect(subject.operator_id).to eq operator.id
      expect(subject.owner_id).to eq operator.id
      assert_in_epsilon operator.cash_on_hand, initial_cash_on_hand - expected_purchase_price, 0.0001
      expect(subject.lease_rate).to be nil
      expect(subject.lease_expiry).to be nil
      expect(Airplane.with_operator(operator)).to include subject
      expect(subject.errors.size).to eq 0
    end

    it "returns false when the current lease has no expiration date" do
  ...
  context "sell" do
    let(:family) { Fabricate(:aircraft_family) }
    let(:model) { Fabricate(:aircraft_model, family: family, floor_space: Airplane::ECONOMY_SEAT_SIZE * 100, takeoff_distance: 10000, speed: 1000, max_range: 13000) }
    let(:owner) { Fabricate(:airline, cash_on_hand: 1000000) }
    ...
    it "does not sell the airplane when it operates routes" do
      game = Game.find(owner.game_id)
      bos = Fabricate(:airport, iata: "BOS", market: owner.base, runway: 10000, exclusive_catchment: 1)
      bos_gates = Gates.create!(airport: bos, game: game, current_gates: 5)
      Slot.create!(lessee_id: owner.id, gates: bos_gates)
    ...
    it "does not sell the airplane when it has no operator" do
      subject.update(operator_id: nil)

      expect(subject.sell).to be false
      expect(subject.errors.full_messages).to include "Operator cannot be empty when selling an airplane"
      expect(subject.owner_id).to eq owner.id
    end

    it "does not sell the airplane when it has no owner" do
      subject.update(owner_id: nil)
    ...
    it "does not sell the airplane when it has not yet been constructed" do
      subject.update(construction_date: Game.find(owner.game_id).current_date + 1.day)
    ...
    it "removes the airplane from the operator's fleet, but does not change its ownership" do
      expect(Airplane.with_operator(owner)).to include subject
      expect(subject.operator_id).to eq owner.id
      expect(subject.owner_id).to eq owner.id
      expect(subject.airplane_routes.count).to eq 0
  ...
  context "terminate_lease" do
    let(:purchase_price_new) { 100000000 }
    let(:initial_cash_on_hand) { 1000000 }
    let(:initial_lease_rate) { 1000 }
    let(:family) { Fabricate(:aircraft_family) }
    let(:model) { Fabricate(:aircraft_model, family: family, floor_space: Airplane::ECONOMY_SEAT_SIZE * 100, takeoff_distance: 10000, speed: 1000, max_range: 13000, price: purchase_price_new) }
    let(:operator) { Fabricate(:airline, cash_on_hand: initial_cash_on_hand) }
    let(:days_remaining_on_lease) { 2 }
    let(:initial_lease_expiry) { Game.find(operator.game_id).current_date + days_remaining_on_lease.days }
    let(:subject) {
      Fabricate(
        :airplane,
        aircraft_model: model,
        ...
        lease_expiry: initial_lease_expiry,
        lease_rate: initial_lease_rate,
        construction_date: Game.find(operator.game_id).current_date - 100.days,
        end_of_useful_life: Game.find(operator.game_id).current_date + model.useful_life * AircraftModel::DAYS_PER_YEAR - 100.days,
    ) }

    it "returns false if the airplane is not yet constructed" do
      subject.update(construction_date: Game.find(operator.game_id).current_date + 1.day)

      expect(subject.terminate_lease).to be false
    ...
    it "returns false if the airline has insufficient cash on hand" do
      operator.update(cash_on_hand: 1)
      ...
      expect(Airplane.with_operator(operator)).to include subject
      expect(operator.cash_on_hand).to eq 1
      expect(subject.errors.full_messages).to include "Operator does not have enough cash on hand to pay the lease termination fee"
    end

    it "returns false if the airplane is still in service" do
      game = Game.find(operator.game_id)
      bos = Fabricate(:airport, iata: "BOS", market: operator.base, runway: 10000, exclusive_catchment: 1)
      bos_gates = Gates.create!(airport: bos, game: game, current_gates: 5)
      Slot.create!(lessee_id: operator.id, gates: bos_gates)
    ...
    it "returns true, removes the airplane from the operator, and resets lease information" do
      ...
      expect(subject.owner_id).to be nil
      expect(Airplane.with_operator(operator)).not_to include subject
      expect(operator.cash_on_hand).to eq initial_cash_on_hand - expected_termination_fee
    end
  end
