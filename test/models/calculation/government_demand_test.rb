...
class Calculation::GovernmentDemandTest < ActiveSupport::TestCase
  ...
  test "demand is zero when origin and destination market are the same" do
    ...
    actual = Calculation::GovernmentDemand.new(micronesia.airports.first, micronesia.airports.last, Date.today).demand(Date.today)
    expected = 0
  ...
  test "demand is zero when the origin is not a national capital" do
    ...
    actual = Calculation::GovernmentDemand.new(micronesia.airports.first, pohnpei.airports.last, Date.today).demand(Date.today)
    expected = 0
  ...
  test "demand is zero when the origin and destination markets are rivals" do
    ...
    actual = Calculation::GovernmentDemand.new(kosrae.airports.first, micronesia.airports.last, Date.today).demand(Date.today)
    expected = 0
  ...
  test "business demand is equivalent to the island demand curve when between islands, domestic, and the demand-maximizing distance" do
    ...
    actual = Calculation::GovernmentDemand.new(pohnpei.airports.first, kosrae.airports.first, Date.today).demand(Date.today)
