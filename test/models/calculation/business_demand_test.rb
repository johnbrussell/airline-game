...
class Calculation::BusinessDemandTest < ActiveSupport::TestCase
  def setup
  ...
  test "demand is zero when origin and destination market are the same" do
    ...
    actual = Calculation::BusinessDemand.new(micronesia.airports.first, micronesia.airports.last, Date.today).demand
    expected = 0

    assert actual == expected
  end

  test "demand is equivalent to the destination population when between islands, domestic, and the demand-maximizing distance" do
    pohnpei = Market.find_by!(name: "Pohnpei")
    kosrae = Market.find_by!(name: "Kosrae")

    actual = Calculation::BusinessDemand.new(pohnpei.airports.first, kosrae.airports.first, Date.today).demand
    expected = 1000

    assert actual == expected
  end

  test "demand is halved when the destination is not an island" do
  ...
  test "demand is reduced by a factor of 12 when the origin is an island and the destination is international" do
  ...
  test "demand uses the island demand curve when the origin is an island" do
    ...
    actual = Calculation::BusinessDemand.new(micronesia.airports.last, kosrae.airports.first, Date.today).demand
